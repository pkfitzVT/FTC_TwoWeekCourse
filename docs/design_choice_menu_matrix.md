...
| Frame / Drivetrain | Chassis geometry | Before frame assembly | `02_student_materials/decision_menus/day2_session4_chassis_shape_menu.md`; chassis physics reading; demo robot observation activity | Robot behavior goals; available materials; field movement; future mechanisms | Labeled top-view chassis sketch and decision record | Motor placement; wheel placement; shooter/hopper space |
...
| Controller Mapping / Driver Controls | D-pad drive or joystick drive | After drivetrain can move safely | `02_student_materials/code_examples/onbot_java/DemoDriveDPad.java`; `02_student_materials/code_examples/onbot_java/DemoDriveJoy.java`; controller mapping comparison activity | Driver comfort; smoothness; lineup accuracy; avoiding contact; test results | Controller mapping decision statement | Driver practice; scoring loop timing; code organization |
...
| Controller Mapping / Driver Controls | Slow mode or normal mode | During drivetrain and scoring tests | Controller mapping activity; drivetrain tests | Aiming accuracy; parking accuracy; overcorrection | Slow-mode decision or stretch goal | Code complexity; driver practice |
